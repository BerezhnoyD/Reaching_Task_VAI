...
const int debounceDelay3 = 5000; // the duration of the Food availability
...
const int but_delay2 = 1000; // the latency in ms for the second sensor to be triggered  - !!! DETERMINES THE TIME ANIMAL SHOULD HOLD THE BAR !!!
...
void loop() {
  ...
  reading4 = digitalRead(inPin4);
    ...
    if (reading1 == 0 && Reinforcement == 0){   // HANDLING THE ROTATION OF THE FEEDER
          Reinforcement = 1;
          Sync = 1;
    ...
    if (Reinforcement == 1 && (millis() - lastDebounceTime2) > debounceDelay3){            // HANDLING THE END OF THE FEEDING
          Reinforcement = 0;
          Sync = 0;
