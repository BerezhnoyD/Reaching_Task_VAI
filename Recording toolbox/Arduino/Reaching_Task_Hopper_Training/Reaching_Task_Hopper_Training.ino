...
const int debounceDelay3 = 10000; // the duration of the Food availability
...
void loop() {
  ...
  Reinforcement = 0;
    ...
    if (reading1 == 0 && (millis() - lastDebounceTime2) > debounceDelay3){   // HANDLING THE ROTATION OF THE FEEDER
          Reinforcement = 1;
    ...
    if ((millis() - cur_time) > Timing){                                                                              // LOGGING THE DATA
        ...
        Serial.print(" ");   
        Serial.print(Reinforcement);     // Sync
        Serial.print(" ");   
