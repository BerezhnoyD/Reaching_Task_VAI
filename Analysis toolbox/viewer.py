...
class ReachesViewer:
    ...
    def __init__(self, file1, file2):
        ...
        self.columns_df= [
                    'group', 'duration_sec', 'dx_mm', 'dz_mm', 'dy_mm', 'dE_mm', 'maxX_mm', 'minY_mm',  
                    'maxZ_mm', 'velocity_mm/sec', 'acceleration_mm/sec^2', 'jerk_mm/sec^3', 'max_velocity_mm/sec', 'max_acceleration_mm/sec^2', 'max_jerk_mm/sec^3']


        self.table = pd.DataFrame(np.array(self.f['data']), columns = self.columns_data)
        self.reach_list = list(self.f['reach_index'])
        self.reach_List = list(str(k)+' '+str(self.reach_list[k]) for k in range(len(self.reach_list)))       
        ...
        for k in range(len(self.reach_all)):
            gr = 1
            dur = self.reach_all[k]['time'].max() - self.reach_all[k]['time'].min()
            dur_std = self.reach_all[k]['time_diff'].std()
            dx = self.reach_all[k]['dx'].abs().sum()                             # The sum of all movements on dX
            ...
            dx2 = self.reach_all[k]['paw_x'].max() # The difference between the max and min dX
            dx2_std = self.reach_all[k]['paw_x'].std()
            dy2 = self.reach_all[k]['paw_z'].max() 
            dy2_std = self.reach_all[k]['paw_y'].std()
            dz2 = self.reach_all[k]['paw_y'].min()
            ...
            list1 = [gr, dur, dx, dz, dy, de, dx2, dz2, dy2, vel, acc, jrk, vel_max, acc_max, jrk_max]
            list2 = [gr, dur_std, dx_std, dz_std, dy_std, de_std, dx2_std, dz2_std, dy2_std, vel_std, acc_std, jrk_std,
                     vel_max_std, acc_max_std, jrk_max_std]
        ...
        self.mean_df.loc[:,'group']='Missed'
        self.std_df.loc[:,'group']='Missed'
        ...
        self.radiobuttons = widgets.RadioButtons(
            value='Missed', 
            options=['Missed', 'In vain', 'Lost', 'Grasped', 'Artefact'], 
            description='Label:'
        )
    ...
    def Plot(self):
        ...
        z = self.table.loc[self.start:self.stop]['paw_y'].reset_index(drop=True)
        y = self.table.loc[self.start:self.stop]['paw_z'].reset_index(drop=True)
        ...
        self.plt1 = self.ax1.plot(x,z,y, linewidth=1, color='grey', label='open')
        self.plt2 = self.ax2.plot(x)
        self.plt3 = self.ax3.plot(z) 
        self.plt4 = self.ax4.plot(y)
