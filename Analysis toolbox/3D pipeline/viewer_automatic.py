...
class ReachesViewer:
    ...
    def __init__(self, file1: str, file2: str, file3: str, options: list):               #Open the h5 file and make a table with scalar parameters for all reaches
        ...
        self.columns_data = ['finger1_x', 'finger1_y', 'finger1_z', 'finger4_x', 'finger4_y', 'finger4_z', 'paw_x', 'paw_y', 'paw_z', 'paw_error', 'paw_score', 'dx', 'dy', 'dz',
        ...
        self.table_classifier = np.array(self.f['data'])[:,:9].T
        
        # make the copy of data, cleared
        self.reach_all = []
        self.cell_tensor = np.empty(shape=(9,70,0))
        ...
        for part in self.reach_list:                #choose the reaches based on the start and end coordinates found on previous stage 
            ...
            self.add_cell_tensor = np.expand_dims(self.table_classifier[:,int(start):int(start)+70], axis=2)
            self.cell_tensor = np.append(self.cell_tensor, self.add_cell_tensor, axis=2)
        ...
        self.cell_tensor = np.reshape(self.cell_tensor, (self.cell_tensor.shape[0]*self.cell_tensor.shape[1],self.cell_tensor.shape[2])).T
        ...
        #load a model and make a prediction using it
        
        self.Cls = pickle.load(open(self.file3, 'rb'))
        self.predict_output = self.Cls.predict(self.cell_tensor)

        self.mean_df.loc[:,'group']=self.predict_output
        self.std_df.loc[:,'group']=self.predict_output
    ...
    def dropdown_eventhandler(self, change):
        ...
        self.radiobuttons.value = self.mean_df.loc[self.current_reach,'group']
