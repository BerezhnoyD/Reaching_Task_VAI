...
def video_crop(x1, y1, x2, y2):                      
    # Script to crop the video of interest
    ...
    out1 = cv2.VideoWriter(filename[:-4]+'-cropped.mp4', fourcc, fps, ((x2-x1),(y2-y1)))
    # Splitting the video in parts
    parts = [(0,frame_count)] # That can be changed if you want to extract the important parts from the video based on timing
    ...
    while (cap.isOpened()):
        ...
        if ret == True:
            ...
            for i, part in enumerate(parts):
                start, end = part
                if start <= f <= end:
                    cam1 = frame[x1:x2, y1:y2]
                    cam1 = cv2.resize(cam1,((x2-x1),(y2-y1)))
                    out1.write(cam1)
        else:
            break
    ...
    cv2.destroyAllWindows()
    print('Done! Your videos are there ' +filename[:-4]+'-cropped.mp4')